...
        distinct：通过流中元素的 hashCode() 和 equals() 去除重复元素*/
        Stream<String> stream23 = Stream.of("hello", "world", "make", "america", "add","again",
                "xi","xidada","add");
        stream23.filter(x -> x.startsWith("a")).limit(6).skip(2)/*.distinct()*/.forEach(System.out::println);
...
//        userStream.sorted((o1,o2) -> {return o1.age - o2.age;}).map(s -> s.name).forEach(System.out::println);
        ...
        */
        ...
        Stream<User> stream12 = users.stream();
        Stream<String> stringStream12 = stream12.peek(x -> x.name = "lilianjie").map(x -> x.name);
        List<String> userList = stringStream12.collect(Collectors.toList());
...
        <U> U reduce(U identity,BiFunction<U, ? super T, U> accumulator,
        ...
        Stream<Integer> stream34 = integers.stream();
        Integer integer = stream34.reduce((s1, s2) -> s1 + s2).get();
        System.out.println(integer);

        Stream<Integer> stream45 = integers.stream();
        Integer reduce = stream45.reduce(10, (s1, s2) -> s1 + s2);
        ...
        List<User> userList3 = Arrays.asList(user1, user2, user3, user4, user5, user6);
        userList3.stream().map(User::getName).collect(Collectors.toList()).forEach(System.out::println);
        Double collect2 = userList3.stream().collect(Collectors.averagingDouble(User::getAge));
        System.out.println(collect2);
        System.out.println(userList3.stream().collect(Collectors.counting()));
        Map<String, List<User>> collect = userList3.stream().collect(Collectors.groupingBy(User::getName));
        System.out.println(collect);
        Map<Boolean, List<User>> collect1 = userList3.stream().collect(Collectors.partitioningBy(v -> v.getAge() > 13));
        System.out.println(collect1);

        userList3.stream().sorted((s1,s2) -> s2.getAge() - s1.getAge()).map(User::getName).forEach(System.out::println);
    }
}
